...
class scrapWeekly(scrapy.Spider):
    ...
    def parse(self, response):
        for item in response.css('div.book_macro_110:not(.recommended_book)'):
            yield {
              ...
              'score': item.css('div.book_metadata_wrapper > p.star_rate > span.RSGBookMetadata_StarRate > span.StarRate_Score::text').get()
            }
